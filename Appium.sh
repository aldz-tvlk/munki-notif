...
def main():
    ...
    if compare_versions(appium_mungki_version, latest_appium_version):
        print(f"Versi baru Appium Inspector tersedia: {latest_appium_version}")
        
        # Perbarui kolom Web Version di file CSV
        update_web_version_csv("Appium", latest_appium_version)
        
        # Kirim notifikasi ke Slack
        send_notification_telegram("Appium", appium_mungki_version, latest_appium_version)
    else:
        print("Versi Appium Inspector sudah yang terbaru.")
